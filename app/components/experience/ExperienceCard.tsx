...
export const ExperienceCard = ({ startDate, endDate, name, description, stack, items }: Props) => {
    return (
                ...
                <div className="flex gap-2 flex-wrap mt-3">
                    {
                        stack.map((item, index) => {
                            return <span className=" flex min-w-20 justify-center items-center rounded-full bg-teal-400/10 px-3 py-1 text-xs font-medium leading-5 text-teal-300 " key={index}>{item}</span>
                        })
                    }
                </div>
            </div>
